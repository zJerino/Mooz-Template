...
document.addEventListener("DOMContentLoaded", () => {
   ...
   * Mz Ripple
   */
  let mzBtns = document.querySelectorAll('.btn');
  let mzBtnEvents = [...mzBtns].forEach((e) => {
    e.addEventListener("click", (event) => {
      ...
      circle.style.left = `${event.clientX - button.offsetLeft - radius + window.scrollX}px`;
      circle.style.top = `${event.clientY - button.offsetTop - radius + window.scrollY}px`;
  
      circle.classList.add("mz-ripple");
    
      const ripple = button.getElementsByClassName("mz-ripple")[0];
